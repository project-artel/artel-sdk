...
### ~~Q1. `ARTEL_AFFORDANCE` define 정책~~ — 해결됨, 그리고 **번복됨** (2026-08-13)

**최종 결정: 게이트를 그대로 유지한다.** 아래에 처음 적었던 "들여오지 않는다"는 취소한다.

번복 근거는 `AffordanceILPostProcessor.cs:21`이다 — 이 분석이 **세 차례 에디터를
먹통으로 만들었고, 그 에디터는 왜 그런지 알아보려 열 수조차 없었다.** 빠져나오는 길은
Unity를 죽이고 매니페스트를 손으로 고치는 것이었다. 같은 파일이 이중 게이트를 의도적
설계로 적고 있고, `project.md`도 "계측은 discovery 모드에서만 켠다"를 제약으로 둔다.

제거를 주장하며 든 근거 셋이 틀렸다:

- "게이트가 아끼는 건 에디터 컴파일 시간뿐" → ②는 게임 어셈블리 무접촉을 보장하고
  그것이 먹통 방지다
- "침묵하는 실패가 최악" → 열리지도 않는 에디터를 만드는 분석기에서는 침묵이 안전한 쪽
- "킬 스위치는 opt-out이어야" → 먹통이 된 에디터에서는 스위치를 끌 수 없다

**SDK 설치와 discovery 실행은 같은 선택이 아니다.** ARTEL-395는 켜고 끄는 수단
(`DiscoveryDefine`·`DiscoveryMenu`)은 이식하고, 첫 임포트에 자동으로 켜는
`FirstImport`만 제외한다.

이 이슈가 옮긴 어셈블리는 원래 게이트가 없으므로 산출물에는 영향이 없다.

---

*아래는 번복 전 기록이다. 판단 과정을 남긴다.*
