...
### Q1. `ARTEL_AFFORDANCE` define 정책 — 결정만 기록

이 이슈가 옮기는 어셈블리는 게이트되지 않으므로 코드에 영향이 없다. 다만 ARTEL-392가
`CodeGen` asmdef을 가져올 때 바로 필요하다.

**제안: 흡수 기간 동안 `defineConstraints`를 그대로 유지한다.** 이미 만들어져 동작하는
장치이고, 무거운 분석 코드를 끄는 탈출구를 남긴다. SDK 신규 설치에서 심볼을 자동으로
넣는 일은 ARTEL-395의 설치기가 맡는다. 정책 자체를 "항상 컴파일"로 바꿀지는 흡수가
끝난 뒤 별도로 판단한다.

이 제안에 이견이 있으면 ARTEL-392 착수 전에 정한다.
