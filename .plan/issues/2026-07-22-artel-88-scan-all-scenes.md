...
- **`DontDestroyOnLoad`는 언로드로 안 사라진다.** 별도 씬이라 `UnloadSceneAsync`가 건드리지 않는다.
  ...
  임시 오브젝트를 넣었다 빼서 얻는다.
- **활성 씬으로 새는 것도 막아야 한다.** 씬 B의 `Awake`/`Start`는 워크가 B를 활성화하기 *전에*
  돌기 때문에, 거기서 `Instantiate`한 오브젝트는 그 시점 활성 씬 — 즉 **게임의 원래 씬** — 에
  들어간다. B를 언로드해도 안 죽고 DDOL도 아니다. 빈 격리 씬(`Artel Scan Quarantine`)을 만들어
  로드·언로드 동안 활성 슬롯을 쥐게 하고, 씬마다 그 루트를 비운다.
...
- **정리가 되돌리지 못하는 것.** 파괴된 매니저가 남긴 `static` 필드·이벤트 구독, 중복을
  자폭시키는 싱글톤 패턴에서 게임 쪽 인스턴스가 먼저 죽은 경우, 이미 나간 오디오·네트워크 요청,
  변경된 `ScriptableObject` 상태. 방문 씬이 게임 오브젝트 밑에 부모로 붙인 것도 게임 씬으로
  옮겨가 구분이 안 된다. 워크 도중 게임이 스스로 만든 오브젝트도 신규로 판정돼 함께 죽는다.
  **청소지 격리가 아니다** — 부작용이 용납 안 되면 에디터 익스포트를 쓴다.
