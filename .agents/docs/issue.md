...
this document and in `workflow.md` applies to the smallest Subtask exactly as it
applies to the 스토리 above it:
...
A one-line change in a second repository still gets its own Subtask, branch, and
PR. The 스토리 never absorbs a Subtask's work, and no Subtask is downgraded to a
