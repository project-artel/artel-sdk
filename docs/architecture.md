...
- `GAME_STATE` 채널 자체는 지금 기본으로 꺼져 있음
  ...
  - ARTEL-400 이 씬 순회를 지우면 스위치도 함께 사라짐
...
- **`pulse` 는 연결이 아니라 세션** — 연결에서 함께 시작하면 안 됨
  - 연결은 모든 씬을 도는 순회도 함께 시작시키고, 그 순회는 아무도 걸어가지 않은 화면을 방문함
  - 독자가 걸러 낼 수도 없음 — 문서 안에 "지금 순회 중" 이라고 적히지 않음
  - 그래서 `start_readings` 가 따로 있고, 그것을 언제 부를지는 런을 모는 쪽만 앎

| 언제 시작했나 | 샘플 게임 실측 | 무엇을 서술했나 |
| --- | --- | --- |
| 씬 순회와 함께 | 8 초에 125,548 바이트 | 플레이어가 있은 적 없는 씬 셋 |
| 순회가 끝난 뒤 | 4,369 바이트 문서 하나, 이어서 14 초 침묵 | 사람이 실제로 있는 화면 |
...
- `Instrument` 컴포넌트가 "이 객체와 그 아래는 계기다" 를 표시하고, 스캔과 순회가 그것을 건너뜀
