...
| 10 초 예산은 분석을 끝나게 하는 장치가 아님 | 모든 루프에 반복 상한이 있어 분석은 어차피 끝남. 이것은 분석을 *곧* 끝나게 함 |
| `WillProcess` 의 prefix 목록에 `Artel` 이 있음 | 제 어셈블리를 그렇게 이름 지은 게임은 조용히 지나쳐짐 |
